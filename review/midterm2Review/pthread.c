...
void run_test1(int num_tasks) {
  ...
  printf("Spawning 1 thread per task(% d tasks)\n", num_tasks);
...
void *worker2(
    void *arg) { // Will run fn workers with the address of the args main sent
  worker_info *thread_data =
      static_cast<worker_info *>(arg); // always do this casting
...
void run_test2(int num_tasks) {
  worker_info
      thread_data; // initializes arg each thread will need to run fn worker2
  thread_data.num_tasks =
      num_tasks; // shared var: but how does it get visible to threads?
  thread_data.next_task =
      0; // shared var: but how does it get visible to threads?
  pthread_t thread[NUM_WORKER_THREADS]; // set an array of four threads
  double start_time = CycleTimer::currentSeconds();
  for (int i = 0; i < NUM_WORKER_THREADS;
       i++) { // every thread runs worker2 with args: thread_data
    pthread_create(&thread[i], NULL, worker2, (void *)thread_data);
  } // threads only run what the main process sends, so need the address of the
    // function, and address of the args
